...
options <- read_excel("options.xlsx",
    col_types = c("date", "numeric", "text",
        "numeric", "numeric", "numeric",
        "numeric", "numeric", "numeric",
        "text", "numeric", "numeric", "numeric",
        "numeric"))
options <- filter(options, ! is.na(options$Exp.Date))
head(options)
tail(options)
```
```{r}
...
option <- mutate(option, Underlying = 1448.75, Today = as.Date("2020-10-07"))
option
```
...
## 2.3 Find those in the money， and calculate their total Open Interest
```{r}
S = 1448.75
...
OTMC <- dplyr::filter(Calls, Strike > S)%>%
    select(`Exp.Date`, Bid, Ask, Strike, Underlying)
OTMP <- dplyr::filter(Puts, Strike < S)%>%
    select(`Exp.Date`, Bid, Ask, Strike, Underlying)
OTMC
OTMP
...
{
    VolP<- OTMP%>%rowwise()%>%
    mutate(volatility = GBSVolatility((Bid+Ask)/2, "p", S, Strike,
    as.numeric(as.Date("2020-12-18") - as.Date("2020-10-07"))/366,
    r = 0.03, b = 0)) %>% ungroup()
  
    VolC <- OTMC%>%rowwise()%>%
    mutate(volatility = GBSVolatility((Bid+Ask)/2, "c", S, Strike,
    as.numeric(as.Date("2020-12-18") - as.Date("2020-10-07"))/366,
    r = 0.03, b = 0)) %>% ungroup()
