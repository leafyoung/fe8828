...
```{r,message=F}
library(fOptions)
df6<-filter(newdata,(OptionType=="p"&Strike<Underlying)|(OptionType=="c"&Strike>Underlying))%>%mutate(price=(Bid+Ask)/2)
df6<-rowwise(df6)%>%mutate(Vol=GBSVolatility(price=price,OptionType,Underlying,Strike,as.numeric(as.Date(Exp.Date)-as.Date(Today))/365,r=0.03,b=0))%>%
  ungroup()%>%arrange(Strike)
