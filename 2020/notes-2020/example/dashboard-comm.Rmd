...
ratio_indicator <- reactive({
  # Quandl does not need a key for free data.
...
output$rollingCorrelation <- renderDygraph({
  ...
  avg <- round(mean(rolling_cor,  na.rm = TRUE), 2)
  mini <- round(min(rolling_cor,  na.rm = TRUE), 2)
  maxi <- round(max(rolling_cor,  na.rm = TRUE), 2)
