...
oil_forecast_data1 <- tibble(
  date = seq(last(index(forebase1)), 
             by = 'week', length.out = 4 * 24 + 1)[-1],
  Forecast = forecast1$mean,
  Hi_80 = forecast1$upper[,1],
  Lo_80 = forecast1$lower[,1])

oil_forecast_xts1 <- xts(oil_forecast_data1[,-1], order.by = oil_forecast_data1$date)
...
oil_forecast_data2 <- tibble(
  date = seq(last(index(forebase2)), 
          by = 'week', length.out = 4 * 3 + 1)[-1],
  Forecast2 = forecast2$mean,
  Hi_80_2 = forecast2$upper[,1],
  Lo_80_2 = forecast2$lower[,1])

oil_forecast_xts2 <- xts(oil_forecast_data2[,-1], order.by = oil_forecast_data2$date)
...
dygraph(oil_combined_xts, main = "Oil Prices: Historical and Forecast") %>%
  ...
  dySeries(c("Lo_80", "Forecast", "Hi_80")) %>%
  dySeries(c("Lo_80_2", "Forecast2", "Hi_80_2"))
