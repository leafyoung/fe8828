...
subtitle: "Week 3: 1. dplyr/2: EDA 2. Building Financial Applications/1"
date: "Oct 1, 2020"
...
df <- full_join(tibble(a = 1:2), tibble(a = 2:4), by = "a")
```
...
```
```{r, echo = TRUE, results = "hide"}
df <- anti_join(tibble(a = 1:2), tibble(a = 2:4), by = "a")
```
...
```{r, echo = TRUE, results = "hide"}
df <- data.frame(dt = c("2019-10-01", "2019-31-12", "2019-03-17", "2019-02-29", "2019-09-30"))
```
...
```{r echo = TRUE, results = "hide"}
wfmt <- data_frame(date = seq(from = as.Date("2019-01-01"), by = "day", length.out = 5),
...
1.1 Copy the options data from
    ...
    Gather data for "Dec 20, 2019" and store into following data frame format.
...
GBSVolatility(867.30, "c", 1135.67, 240,
              as.numeric((as.Date("2019-12-20") - as.Date("2019-09-16")))/365, r = 0.03, b = 0)
...
Don't take granted that all R functions are able to take in vector and output vector. GBSVolatility can only take in single number, not vector.
