...
```{r echo = TRUE}
S1 <- 98
option_pnl <- GBSOption("c", S = S1, X = 100, Time = (days - 1) / 250, r = 0, b = 0, sigma = 0.3)@price - price0
...
sum(0.5 * opt$gamma * (0.5 ** 2 - 0.3 ** 2) * timestep * timestemp * price * price, na.rm = TRUE)
sum(opt$delta * lead(opt$delta_S), na.rm = TRUE) - opt$price[1] + opt$price[20]

res <- replicate(1:5000, {
  ss <- rep(S, N) * c(1, cumprod(rlnorm(N - 1, mean = p1, sd = p2)))
