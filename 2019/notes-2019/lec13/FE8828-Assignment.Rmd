...
[2] And overall Profit and Loss "overall PnL" for the strategy is $\approx Vega * (Realized \space Vol - Implied \space Vol) * T$. The overall PnL needs to count for the cost of buying the option.
...
opt_price <- GBSOption("c", S = S, X = 100, Time = days / 250, r = 0, b = 0, sigma = 0.3)@price
delta <- GBSGreeks("Delta", "c", S = S, X = 100, Time = days / 250, r = 0, b = 0, sigma = 0.3)
...
replicate(500,
  {
    ss <- rep(S, N) * cumprod(rlnorm(N, mean = p1, sd = p2))
    pnl <- (-1) * opt_price + max(0, ss - K) + (-1) * delta * (ss - K)
    pnl
  }) -> aa

vega_pnl <- vega * (sigma - 0.3) * sqrt(20 / 250)
vega_pnl
