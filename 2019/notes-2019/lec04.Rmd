...
subtitle: "<sub> Session 2 <br> Intermediate R Programming <br> Shiny/2: R Web Framework <br> Data Manipulation and EDA (Exploratory Data Analysis)/1 </sub>"
...
# Lecture 4: Intermediate R Programming
Let's review some R basics and cover some intermediate R programming.
...
3. Remember the best number from evaluaton group and match the first number in selection group, >= than best.
4. Returns this number

Run this function for a few (hundred) times and find the probability of getting N

Step 2: find_optimal, calls Step 1 a few (hundred) times for each of the split number from 1 to N/2.
So we can find the optimal value for the split for the N.

Answer: I suggest to find the solution for N = 3, and N = 10, then move straight on to N = 100.
...
# SQL
- Let's briefly mention SQL, which first appeared in 1974, which is 43 years ago.
...
# dplyr
dplyr package from tidyverse is a high-performance package to deal data in data frame.
