#Question: Shiny app tweak (shiny-24.R)
...
ui <- fluidPage(
  ...
  fluidRow(
    column(8, h3("dt2"),
           dataTableOutput("dt2")),
    column(4, h3("p5"),
           #following is the selectInput added for the assignment
           selectInput("colorname", "Select dot color:", "red", choices = colours()),
           plotOutput("p5")))
)
...
server <- function(input, output, session) {
  ...
  output$p5 <- renderPlot({
    ...
    if (length(s)) {
      #Use the color selected from user
      points(iris[s, c("Sepal.Length", "Sepal.Width"), drop = F],
             pch = 19, cex = 1, col = input$colorname)
    }
  })
}
shinyApp(ui, server)