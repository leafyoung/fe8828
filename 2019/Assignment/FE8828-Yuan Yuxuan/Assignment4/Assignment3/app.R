# shiny-36-update.R
# Assignment 3
# Shiny: add remove button. shiny-36-update.R
library(shiny)
ui <- fluidPage(
    numericInput("shock", "Shock", value = round(runif(1) * 1000), 0),
    actionButton("add", "Add"),
    actionButton("remove", "Remove"),
    actionButton("removeallothers", "Remove Others"),
...
server <- function(input, output, session) {
    ...
    observeEvent(input$remove, {
        shock <- isolate(input$shock)
        if (shock %in% scenarios) {
            scenarios <<- sort(setdiff(scenarios, c(shock)))
            updateCheckboxGroupInput(session, "scenarios",
                                     choices = scenarios,
                                     selected = scenarios)
        }
    ...
    observeEvent(input$removeallothers, {
        shock <- isolate(input$shock)
        if (shock %in% scenarios) {
            scenarios <<- c(shock)
            updateCheckboxGroupInput(session, "scenarios",
