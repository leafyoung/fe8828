...
for(i in 1:12) {
  ...
  start_volatility <- as.numeric(xts_obj[start_date,"IV30"]) / 100
  ...
  df_opt <- rowwise(df) %>%
    #option_premium
    mutate(strike = as.numeric(start_price),
           volatility = start_volatility,
           premium = GBSOption(TypeFlag = "c", 
                               S = Close,
    ...
    ungroup %>% 
    #Option_DoD_Pnl
    mutate(Option_DoD_PnL = quantity * ifelse(date == start_date, 
                                   # premium * (-1), 
                                   0,
    ...
    ungroup() %>%
    #Heding_DoD_PnL
    mutate(Hedging_DoD_Pnl = ifelse(date == start_date, 0, dplyr::lag(delta_hedge) * (Close - dplyr::lag(Close)))) %>%
