...
# working_directory = "~/Dropbox/FE8828-Yuan Yuxuan/GroupProject/Question1/"
working_directory = "E:/gdrive/MFECourse/FE8828/2019/Group/Group 5/Q_1/"
...
CurrencyInfo <- read.csv(
    paste0(working_directory, "Exchange Rates.csv"), header = TRUE)
...
server <- function(input, output) {
    ...
    output$bankPlot <- renderPlot({
        if(!is_empty(values$pnl)){
            tmp <- values$pnl %>%
                dplyr::select(Date, Deposit, Credit) %>%
    ...
    observeEvent(input$clientgo, {
        values$accountSummary = aggregate(. ~ TransactionType, 
                                          dplyr::select(subset(Client, 
                                                               format.Date(Date, "%m")== input$clientMonth
                                                               & Name == input$client
                                                               & TransactionType != "Clear"),
                                                        TransactionType, SGAmount), sum)
        values$monthEndBalance = dplyr::select(subset(Client, TransactionType == "Interest" & format.Date(Date, "%m")==input$clientMonth & Name == input$client), Deposit, Credit)
        values$transactionDetails = dplyr::select(subset(Client, format.Date(Date, "%m")== input$clientMonth 
                                                         & Name == input$client), 
        ...
        transactionDetails = dplyr::select(subset(Client, format.Date(Date, "%m")== input$clientMonth 
                                                  & Name == input$client), 
                                           Date, Deposit, Credit)
    ...
    observeEvent(input$bankgo, {
        ...
        for(name in unique(AccountInfo$Name)){
            transactionDetails = dplyr::select(subset(Client, format.Date(Date, "%m")== input$bankMonth 
                                                      & Name == name), Date, Deposit, Credit, PnL, SGAmount, TransactionType)
            mergeDF = merge(allDays, transactionDetails, by="Date", all = TRUE)
        ...
        values$pnl = dplyr::select(aggregate(. ~ Date, pnlTable, sum), Date, Deposit, Credit, PnL)
        riskT = dplyr::select(subset(Client, TransactionType == "Interest" & format.Date(Date, "%m")==input$bankMonth), Name, Deposit, Credit)
