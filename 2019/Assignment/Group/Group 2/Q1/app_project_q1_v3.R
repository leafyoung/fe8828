...
setwd("E:/gdrive/MFECourse/FE8828/2019/Group")
...
#2
# currency <- read.csv("C:\\Users\\AAA\\Desktop\\Assignments\\Web application\\Project\\Exchange Rates.csv", stringsAsFactors = F)
currency <- read.csv("E:/gdrive/MFECourse/FE8828/2019/Group/Group 2/Q1/Exchange Rates.csv", stringsAsFactors = F)
currency <- mutate(currency, End.of.Period = 2019)
# insert "month" for each observation
for (i in 2:nrow(currency)){
    if (currency$X[i] == ""){
        currency$X[i] <- currency$X[i-1]
    }
}
currency <- mutate(currency, Date = ymd(paste0(End.of.Period, X, X.1)))  
...
# Extrapolate exchange rate for weekends. we use Friday's exchange rate for Saturday and Monday's rate for Sunday
...
# Extrapolate exchange rate for holidays. use the rate at the day before holiday
for (i in 1:nrow(Currency)){ 
    if (is.na(Currency$weekdays[i])){
...
# the input parameter of get_client() is the account number of the client
get_client <- function(t){
    ...
    # transaction status is initialized as "succeed" and later we will check whether the transcation actually succeed
    # the exchange rate is set to 1(representing SGD)
    ...
    client_data$balance[1] <- initial_random_deposit$deposit[t] * (1 + r) + client_data$amount[1]
    client_data$interest[1] <- -(initial_random_deposit$deposit[t] * r + client_data$interest[1])
    # in the following for loop, we run through the entire dataframe and calculate all 
    # the values of deposit, withdraw, and spend altoghther
    for (j in 2:nrow(client_data)){
        if (client_data$TransactionType[j] == "deposit"){
            ...
            # we assume that at the beginning of each month, the credit will be restored to 0                            interest is set to negative previously
            ...
            # if the spending amount exceeds the credit, the transaction will not be executed
            ...
            # spend only affects credit, so the balance remains unchanged
            ...
            # withdray only affects balance, so the credit remains unchanged
            client_data$credit[j] <- client_data$credit[j-1]
        }
    }
    return(client_data)
...
Transcation_Chart <- function(client, month){
    ...
    client_month <- dplyr::filter(client_all, AccountNo == client, month(date) == month)
    client_month_2 <- dplyr::filter(client_month, TransactionType == "deposit")
...
Transaction_History <- function(client, month){
    client <- parse_number(client)
    client_month <- dplyr::filter(client_all, AccountNo == client, month(date) == month, amount != 0)
...
Transaction_Summary <- function(client, month){
    client <- parse_number(client)
    dplyr::filter(client_all, AccountNo == client, month(date) == month, amount != 0) %>%
...
Bank_chart <- function(month){
    bank_month <- dplyr::filter(client_all, month(date) == month, amount != 0, TransactionType == "withdraw")
...
Bank_pnl <- function(month){
    ...
    bank_month_2 <- dplyr::filter(client_all, month(date) == month, amount != 0) %>%
        mutate(date = as.character(date))
...
Bank_risk_table <- function(month){
    end_month <- dplyr::filter(client_all, month(date) == month, day(date) == days_in_month(month(date)), TransactionType == "withdraw")
    ...
    end_month <- select(end_month, `Client Name`, Deposit = balance, Credit = credit)
    return(end_month)
}
