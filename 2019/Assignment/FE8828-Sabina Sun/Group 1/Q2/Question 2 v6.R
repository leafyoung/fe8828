...
#Define variables
  ...
  for (run in 1:1000) {
    df <- genranS(S,sigma)
    opt1 <- creategreeks(df,sigma)%>% ungroup %>%
  ...
  for (run in 1:1000) {
    ...
    opt3 <- creategreeks(df3,sigma) %>%
      mutate(shareHedge = - numOptions * delta,
             ...
             #                        -S * (shareHedge - dplyr::lag(shareHedge))),       
             DoD_PnL = ifelse(time == .$time[1],  0,
                              dplyr::lag(shareHedge) * (S - dplyr::lag(S)) ),
             
             # Use previous gamma
             approxpnl = numOptions * (0.5 * dplyr::lag(gamma) * S^2 * dt  * (realizedVol^2 - sigma^2))
      )
      ...
      finalstockvaluevec <- c(finalstockvaluevec, 0)
