...
scenarios <- c(-100, -50, 0, 50, 100) #prepopulate with these numbers.
...
server <- function(input, output, session) {
  ...
  observeEvent(input$remove,{
    shock <- isolate(input$shock)
    if (shock %in% scenarios){
      index_removed <- (which(scenarios %in% c(shock)))
      # removing the index, <<- because session object, may not use normal assignment.
      scenarios <<- scenarios[-index_removed] # updates the session.
      updateCheckboxGroupInput(session, "scenarios",
                               choices = scenarios,
                               selected = scenarios)
    }
