...
Q2P1: This cell will show how accurate the Taylor's approximation is if applied to actual PnL.
...
Q2P2: If the realised volatility and implied volatility is the same, `sum(0.5*Gamma*deltaS^2+dt_Theta*dt)` approximates to 0.
...
# There are negatives in the individual terms but the total summation is always nonnegative.
```
...
ggplot(df_melt, aes(x=date, y=value, col=`PnL Types`)) + 
  geom_line() + xlab("Date") + ylab("PnL") + ggtitle("Options, Stock and Cumulative PnL")+ theme(plot.title = element_text(hjust = 0.5)) +
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
```

Analysis: Option and stock daily PnL against the time to expiry. This shows us the daily fluctuation in PnL.
```{r}
...
ggplot(df_melt, aes(x=date, y=value, col=`Daily PnL Types`)) + 
  geom_line() + xlab("Date") + ylab("PnL") + ggtitle("Daily PnL against Time to expiry (Overall)")+ theme(plot.title = element_text(hjust = 0.5)) +
   theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
```

Analysis: Option and stock daily PnL against the time to expiry. This plot splits the daily PnL flunction into monthly basis. This means different colored lines represents the fluctuation in PnL in that particular month.
```{r}
# Track PnL against time to maturity.
...
ggplot(df_melt, aes(x=date, y=value, col=`Year/MM`)) + 
  geom_line() + xlab("Date") + ylab("Daily PnL") + ggtitle("Daily PnL against time to expiry (Monthly)")+ theme(plot.title = element_text(hjust = 0.5))+
  theme(axis.text.x=element_text(angle=45, vjust=.8, hjust=1))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black"))
```

Backtesting for 1year, closing options and stocks monthly, Delta Hedge with 100 put options with Delta 0.25. 
...
Analysis: Option and stock daily PnL against the time to expiry. This plot splits the daily PnL function into monthly basis. This means different colored lines represents the fluctuation in PnL in that particular month.
