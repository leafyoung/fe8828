...
rowwise(OptData) %>% mutate(b = 0, r = 0.01, underlyingPx = Px) %>%
 dplyr::filter((`Call/Put` == "c" & Strike > Px) | (`Call/Put` == "p" & Strike < Px)) %>%
  mutate(IV = GBSVolatility(price = Price, TypeFlag = `Call/Put`, S = underlyingPx, X = as.numeric(Strike), Time = as.numeric(ExpDate - Date), r =r, b = b)) %>% select(Strike, IV) %>% plot(type = "l")
