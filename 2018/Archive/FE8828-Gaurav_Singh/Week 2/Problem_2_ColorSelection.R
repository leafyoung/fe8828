...
ui <- fluidPage(h3("t1"),
  ...
  fluidRow(
    column(8, h3("dt2"),
           dataTableOutput("dt2")),
    column(4, h3("p5"),selectInput(inputId="colorSelected","Color chosen points with :",colours(),selected = "red"),
           plotOutput("p5")))
)
...
server <- function(input, output, session) {
  ...
  output$dt2 <- renderDataTable(iris, options = list(pageLength = 5),server = FALSE)
  output$p5 <- renderPlot({
    ...
    if (length(s)) {
      points(iris[s, c("Sepal.Length", "Sepal.Width"), drop = F],
             pch = 19, cex = 1, col = input$colorSelected)
    }
  })
}
shinyApp(ui, server)