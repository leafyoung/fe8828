...
```
  ...
  As can be seen from the graphs, we get extreme and volatile final PnL when average prices are about 230 and 260, where the density function of stock prices also peaked around the same level. As we explore further, these extreme pnl points occur only when start price is close to 220 and end price is close to 237.5, or start price is close to 250 and end price is close to 268. 
  Comparing the the hedging and option PnL, we clearly see a hedging relationship between option and stock position in this strategy.There also seems to exist a linear relationship between final p&L and average growth rate of stock, which is consistent with our expectation.
