...
server <- function(input, output, session) { 
  ...
  observeEvent(input$remove,{
    ...
    if (shock %in% scenarios) {
      remove <- c(shock)
      scenarios <<- scenarios[!scenarios %in% remove]
      updateCheckboxGroupInput(session, "scenarios",
                               choices = scenarios,
                               selected = scenarios)
    }
    else{
      msg = paste0(shock, " is not found in the scenarios. Please try again.")
    ...
    # a new random value is not meaningful for remove function, hence no need to reset the input
  })
