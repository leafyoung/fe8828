...
```{r, echo = F, out.height = '20%', out.width="100%"}
...
```{r, echo = F, out.height = '30%', out.width = '100%'}
...
# `g`: mix and match
- Please find the matched figure to the command

```{r, echo = F, out.height="100%", out.width="100%"}
p1 <- g + geom_point(aes(color = job), size = 4, alpha = 1/2)
p2 <- g + geom_point() + geom_point(aes(color = job), size = 4, alpha = 1/2) 
p3 <- g + geom_point(color = "steelblue", size = 4, alpha = 1/2)
p4 <- g + geom_point() + geom_smooth(method = "lm") + facet_grid(. ~ job)
...
# Exercise 
2. I tried to plot between job, loan and age. Better idea? 
...
# Density: balance and job wtih fill
```{r, echo = T}
ggplot(bank, aes(duration, fill = job)) + geom_density()
```

# Density: age and job wtih alpha
```{r, echo = T}
