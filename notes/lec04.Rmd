...
another_young <- filter(bank, age < 20 & marital == "married")
just_young <- filter(bank, age < 20 & marital == "single")

young2 <- filter(bank, age >= 20 & age < 30)
another_young2 <- filter(bank, age >= 20 & age < 30 & marital == "married")
just_young2 <- filter(bank, age >= 20 & age < 30 & marital == "single")
```
...
second_upper <- filter(bank, education %in% c("tertiary", "secondary"))

# filter out NA value.
no_na <- filter(bank, is.na(balance) | balance > 0)
```
...
# Summary
- We learned the key "verbs" from dplyr. The rest will be picked up next week.
