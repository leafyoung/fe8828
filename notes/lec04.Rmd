...
# Matching/Extracting with Regular Expression (RE)
## Matching
```{r, echo = T}
sub("[^\\_]+\\_.*", "", "USDCNY_M1")
```
...
- %m: month in number
- %b/%B: month in abbr./full. i.e. Jan/January.
- %d: day
...
# Exercise
* Write a function to determine leap year?
* Print the list of Month names
...
# Map
basket <- sample(c("Apple", "Orange", "Banana", "Pear"), 100, replace = T)
fruit_count <- list()
for (b in basket) {
...
fruit_count <- add_to_map(fruit_count, b, 1)
...
sapply(opts, function(o) {
  (GBSOption(o@type, o@underlying, o@strike, Time = 1, r = 0.01, b = 0, sigma = 0.3))@price
}))
cat(as.numeric(Sys.time() - a))

# alternative to sapply, we can use map* functions from `purrr` package
# `map` is a generic function that returns a list
# `map_dbl` is for result of double, it would return a vector
purrr::map_dbl(opts, function(o) {
  (GBSOption(o@type, o@underlying, o@strike, Time = 1, r = 0.01, b = 0, sigma = 0.3))@price
})
```
...
# For more complicate, you can make it multilines
(function(x) {
  if (x > 3) {
    print(x)
  } else {
...
purrr::map(1:10, function(x) rnorm(x), n = 10)
# function(x) func(x) can be simplied as func.
purrr::map(1:10, rnorm, n = 10)
# This is what we want to do this. %>% means to use previous result.
...
# Read/Write data
- `setwd("C:/TEMP")` 
- `saveRDS(object, file = "C:/TEMP/DATA/data.Rds")`
- `x <- readRDS(file = "C:/TEMP/DATA/data.Rds")`

- On Windows, use double slash `\\` or single backslash `/`. e.g. `C:\\TEMP\\DATA`, `C:/TEMP/DATA`
- On Mac, use `/Users/.../`
...
```{r, echo = T}
df <- data.frame(a = 1:10, b = 10:1, c = runif(10), d = sample(LETTERS, 10))
...
result <- sapply(seq(1:1000000), sample_bday, simplify = TRUE)

# library(parallel)
# cl <- setDefaultCluster()
# result <- parSapply(seq(1:1000), sample_bday, simplify = TRUE)
...
sim_sample <- replicate(1000000, survi())
...
print(paste("res_sim:", res_sim))
print(paste("res_ana:", res_ana))
hist(sim_sample)

library(ggplot2)
qplot(sim_sample, geom="histogram", binwidth = 1) 
```
...
All input function follow such function signature except for later parameters.
...
# Output
      ...
      plotOutput(outputId = "hist"): renderPlot({ plot(runif(1000), runif(1000)) })
      tableOutput("t1"): output$t1 <- renderTable(iris)
      dataTableOutput("t2"): output$t2 <- renderDataTable(iris)
      htmlOutput("h1"): output$h1 <- renderUI({ tagList(
...
i.e. Put input$XXX inside renderXXX
...
- Reactive- wiring things
- Connect from trigger- button, isolate to create a Chinese wall
- Connect from receiver- plot/table to data

# Assignment (Dates)
* Create a Bond Schedule
* Craete a time series
...
Library dplyr::lag is creating a conflict.
